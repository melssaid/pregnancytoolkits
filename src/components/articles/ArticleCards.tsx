    ...
    <Link to={`/articles/${article.slug}`} className="block">
      <Card className={cn("group overflow-hidden rounded-[1.15rem] transition-all duration-300", tone.shell)}>
        <div className="relative space-y-2.5">
          ...
          <div className="mx-1 rounded-[1rem] border border-border bg-card px-3 py-3">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0 flex-1">
                <h3 className="line-clamp-2 text-[15px] font-black leading-7 text-foreground ar-heading">{article.title}</h3>
                <p className="mt-1 line-clamp-3 text-[13px] leading-6 text-foreground">{article.excerpt}</p>
              </div>
