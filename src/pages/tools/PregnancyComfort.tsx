...
function NauseaTab() {
  ...
  return (
    <div className="space-y-4">
      ...
      <motion.button whileTap={{ scale: 0.92 }} onClick={getReliefPlan} disabled={isLoading}
        className="relative w-full overflow-hidden rounded-xl h-11 flex items-center justify-center gap-2 text-white text-sm font-semibold shadow-lg disabled:opacity-60"
        style={{ background: "linear-gradient(135deg, hsl(var(--primary)), hsl(330 70% 55%), hsl(280 60% 55%))" }}>
        {isLoading ? <><Loader2 className="w-4 h-4 animate-spin shrink-0" /><span>{t('toolsInternal.nauseaRelief.creatingPlan')}</span></> : <><Brain className="w-4 h-4 shrink-0" /><span>{t('toolsInternal.nauseaRelief.getReliefPlan')}</span></>}
      </motion.button>
